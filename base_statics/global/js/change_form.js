...
document.addEventListener('DOMContentLoaded', function() {
  // pega o botão de adoção na change_form
  var btnAdotar = document.querySelector('.marked-button');
  if (!btnAdotar) return;

  btnAdotar.addEventListener('click', function(e) {
    e.preventDefault();
    openCrudModal({
      title: 'Marcar como adotado',
      contentHtml: '<p>Tem certeza que deseja marcar este animal como adotado?</p>',
      buttons: [
        {
          text: 'Cancelar',
          className: 'instagram-btn',
        ...
        },
        {
          ...
          className: 'whatsapp-btn',
          onClick: function() {
            window.location.href = btnAdotar.dataset.url;
          }
        }
      ]
    });
...
document.addEventListener('DOMContentLoaded', function() {
  var btnRevert = document.querySelector('.revert-button');
  if (!btnRevert) return;

  btnRevert.addEventListener('click', function(e) {
    e.preventDefault();
